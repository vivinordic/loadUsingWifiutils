#-------------------------------------------------------------------------------
# Name:        aaci
# Purpose:     This script will calculate the aaci value for the specified datarate.
...
def get_10prcnt_per(ch,dr,start_amp,end_amp,step_size,min_per_val,max_per_val,a):
    ...
    for amp in dig_amplitude_list[1:]:
        ...
        if(float(curr_per)>HIGH_PER_BOUNDARY):
            ...
            aaci_amp_dict[ch][dr]=float(-1*((float(sensitiviy_dr[dr]))-(float(curr_amp)-cable_loss)))
        ...
        if(flg==0):
            ...
            if(LOW_PER_BOUNDARY<=float(curr_per)<=HIGH_PER_BOUNDARY):
                if(ampl_dict[dr].index(amp)>0):
                    flg=1
                    ten_perc_per_sig=str(curr_amp)
                    aaci_amp_dict[ch][dr]=float(-1*((float(sensitiviy_dr[dr]))-(float(amp)-cable_loss)))
                    worksheet.write_row('A'+str(row_num), data)
                    worksheet_dr.write_row('A'+str(row_num_dr), data)
                    row_num_dr=row_num_dr+1
                    axis_num=axis_num+1
...
def save_to_doc(hdng,freq,chn,bw):
    ...
    document_path=os.path.join(op_file_path.split(standard)[0],'RX_Characterization_AACI_'+release+'.docx')
    try:
    ...
    if('AACI of All DataRates' not in paragraphs):
        document.add_heading('AACI of All DataRates', level=2)
    if(standard=='11ax'):
    ...
    for datarate in data_rate.split(','):
        if(datarate in aaci_amp_dict[chn].keys()):
            row_cells = table.add_row().cells
            row_cells[0].text = str(datarate)
            if('TBT' in str(aaci_amp_dict[chn][datarate])):
                row_cells[1].text = str(aaci_amp_dict[chn][datarate])
            else:
                row_cells[1].text = str(round(float(aaci_amp_dict[chn][datarate]),2))
    try:
...
def aaci_analysis(DUT_TestConfigParams,dut1):
    #controlPowerSwitch(on_ports_list='8',off_ports_list='8')
    global ftime
    global equipment
    global dut
...
##    dut=eval(dutModel)(com_port)
    ...
    for dr in data_rate.split(','):
        ...
        print(sensitiviy_dr)
        start_amplt= sensitiviy_dr[dr]+3+cable_loss+ 75 #aaci_dict_range[standard][streams][bw][dr][0]
        end_amplt=start_amplt - 50 #aaci_dict_range[standard][streams][bw][dr][1]
    ...
    for ch in channel.split(','):
        ...
        if(standard=='11ac'):
            ...
            #cnsl_fname=''
        consld_fname=os.path.join(op_file_path,'AACI_Consolidated_'+cnsl_fname+'.xlsx')
        workbook = xlsxwriter.Workbook(consld_fname)
        worksheet = workbook.add_worksheet()
        bold1 = workbook.add_format({'bold': 1})
        if(ch not in aaci_amp_dict.keys()):
            aaci_amp_dict[ch]={}
        for dr in data_rate.split(','):
            aaci_amp_dict[ch][dr]='TBT'
            ...
            dr_file_name=op_file_path+'\AACI'+fname+'.xlsx'
            ...
            if (DUT_TestConfigParams.in_band_pkt == 1):
                if(standard=='11b'):
                    equipment.set_amplitude(streams,str(sensitiviy_dr[dr]+3+cable_loss))#TODO Adjustment AS per Sensitivity Values with Splitter, +10 is given for debugging remove later
                else:
                    equipment.set_amplitude(streams,str(sensitiviy_dr[dr]+3+cable_loss))#TODO Adjustment AS per Sensitivity Values with Splitter, +10 is given for debugging remove later
            else:
