...
def amplitude_loop(dr,ch,workbook,worksheet,standard,cable_loss,DUT_TestConfigParams,staID,test_no):
    ...
    for amp in ampl_dict[dr]:
        ...
        data.append(int(ch))
        data.append(DUT_TestConfigParams.bw)
        data.append(DUT_TestConfigParams.p20_flag)
        if DUT_TestConfigParams.standard !='11ax':
            ...
            data.append(hex(DUT_TestConfigParams.ruAllocation))
            data.append(staID)
            data.append(DUT_TestConfigParams.userRuIndex)
            data.append(DUT_TestConfigParams.staIDlist)
            data.append(DUT_TestConfigParams.stbc)
            data.append(DUT_TestConfigParams.coding)
        ...
        if(flg==0):
            ...
            data1 = start_per(modulation,dr,ch,vsg_streams,standard,tester,dut,equipment)
            data=data + data1
            if(data1[0] < 800 ):# and ((ampl_dict[dr].index(amp)==0) or (ampl_dict[dr].index(amp)==1))):
                DUT_functions.exitRxWaitLoop()
                for i in range(num_params_to_pop):
                    data.pop()
                data1 = start_per(modulation,dr,ch,vsg_streams,standard,tester,dut,equipment)
                data=data + data1
            if((data1[0] > 3000) and ('IQXEL' in tester)):
                time.sleep(2)
                for i in range(num_params_to_pop):
                    data.pop()
                data1 = start_per(modulation,dr,ch,vsg_streams,standard,tester,dut,equipment)
...
def rx_analysis(DUT_TestConfigParams,dut1,test_no):
    #controlPowerSwitch(on_ports_list='8',off_ports_list='8')
    global ftime
    global equipment
    global dut
    ...
    consld_fname=os.path.join(op_file_path.split(standard)[0],'RX_functional_Consolidated_results.xlsx')
    print(consld_fname)
    ...
    if os.path.exists(consld_fname):
        ...
        workbook = Workbook()
        worksheet = workbook.active
        worksheet.append(['Testcase No.','Standard','Channel','BW','P20_offset','RU Allocation','RU Allocation Hex','STAID','userRuDistribution','staIdList','stbc',\
         'coding', 'doppler', 'giType', 'dcm', 'midamblePeriodicity', 'heLtfType',\
          'sigBMCS', 'sigBDCM', 'sigBCompression', 'Comment','DataRate','AMPLITUDE']+rx_report_hdngs)
        ...
        if DUT_TestConfigParams.standard =='11ax':
            for ii in range(DUT_TestConfigParams.numUser):
                ...
                for staID in desiredStaIdList:
                    ...
                    amplitude_loop(dr,channel,workbook,worksheet,standard,cable_loss,DUT_TestConfigParams, staID,test_no)
        else:
            equipment.set_datarate(modulation,standard,dr)
            equipment.generate_waveform(streams=vsg_streams)
            ftime.write("AMP LOOP START"+time.strftime("%H-%M-%S")+"\n")
            amplitude_loop(dr,channel,workbook,worksheet,standard,cable_loss,DUT_TestConfigParams,0,test_no)
