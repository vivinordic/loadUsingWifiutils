...
def amplitude_loop(dr,ch,workbook,workbook_dr,worksheet,worksheet_dr,standard,cable_loss):
    start_amp_list=[]
    global row_num
    ...
    for amp in ampl_dict[dr]:
        ...
        if(flg==0):
            ...
            if(data[4] < 800 ):# and ((ampl_dict[dr].index(amp)==0) or (ampl_dict[dr].index(amp)==1))):
                time.sleep(1)
                dut.set_dut_channel(ch,p20_flag)
        ...
        per_amp_dict[dr].append(float(data[-1]))
        row_num=row_num+1
...
#Plot PER for single data rate sheet
def plot_dr_per(dr,workbook_dr,worksheet_dr):
    ...
    chart_dr.add_series({
        ...
        'values':  '=Sheet1!$AF2:AF'+str(len(amplitude_list)+1)
    })
    ...
    worksheet_dr.insert_chart('AG2', chart_dr, {'x_offset': 5, 'y_offset': 5})

#Plot PER for all data rates in a single sheet
def plot_all_per(workbook,worksheet,ch=''):
...
def save_to_doc(hdng,imag,freq,chn):
    ...
    if (fadingtype != 'NA'):
        ...
        document_path=os.path.join(op_file_path.split(standard)[0],'RX_Characterization_'+release+'_'+board_num+'_'+rf_num+'.docx')
    print(document_path)
    try:
    ...
    if('PER Bathtub Curves' not in paragraphs):
        document.add_heading('PER Bathtub Curves', level=2)
    if(standard=='11ac'):
...
def plot_per_png():
    ...
    for amp in amplitude_list:
        per_plot_list.append(amp-cable_loss)
    for dtr in data_rate.split(','):
        plt.plot(per_plot_list,per_amp_dict[dtr],label=dtr,linewidth=3.0)
    ax=plt.gca()
    ax.invert_xaxis()
    if(standard=='11ac'):
    ...
    if (fadingtype != 'NA'):
        cnsl_fname += '_fadeType_' + fadingtype
...
def per_analysis(DUT_TestConfigParams,dut1):
    #controlPowerSwitch(on_ports_list='8',off_ports_list='8')
    global ftime
    global equipment
    global dut
    ...
    print "In per_analysis() of per.py\n "
...
##        exit(0)
    ...
    if(standard=='11ac'):
        ...
        # cnsl_fname=''
    consld_fname=os.path.join(op_file_path,'PER_Consolidated_'+cnsl_fname+'.xlsx')
    if (DUT_TestConfigParams.fadingtype != 'NA'):
        consld_fname += '_fadeType_' + DUT_TestConfigParams.fadingtype
    print(consld_fname)
...
##    DUT_functions.startTestcase() # PHY_PERFORMANCE - this was missing and is a bug - without this DUT test wont start
    ...
    for dr in data_rate.split(','):
        ...
        if (DUT_TestConfigParams.fadingtype != 'NA'):
            fname += '_fadeType_' + DUT_TestConfigParams.fadingtype

        dr_file_name=os.path.join(op_file_path,'Per_'+fname+'.xlsx')
        ...
        amplitude_loop(dr,channel,workbook,workbook_dr,worksheet,worksheet_dr,standard,cable_loss)
        ftime.write("AMP LOOP END"+time.strftime("%H-%M-%S")+"\n")
        plot_dr_per(dr,workbook_dr,worksheet_dr)
        workbook_dr.close()
    plot_all_per(workbook,worksheet,ch=channel)
    try:
        workbook.close()
    except:
        time.sleep(10)
