...
def amplitude_loop(dr,ch,workbook,worksheet,standard,cable_loss,DUT_TestConfigParams,staID):
    ...
    for amp in ampl_dict[dr]:
        ...
        if(flg==0):
            ...
            if (DUT_TestConfigParams.jammerOn == 1):
                equipment.rf_on_off_jammer(rf_state='on',streams=streams,chain_sel=2)
            if (DUT_TestConfigParams.packetOn == 1):
                data=data+start_per(modulation,dr,ch,vsg_streams,standard,tester,dut,equipment)
            else:
                stats =  dut.get_negative_stats()  # If packet not present RX stats directly read from peripheral registers
                data = data+ [stats[0], 0,0,stats[0],0] +stats[1:5] +[0]+stats[5:]+ [0]*6 + [100]
...
def rx_jammer_analysis(DUT_TestConfigParams,dut1):
    ...
    consld_fname=os.path.join(op_file_path.split(standard)[0],'RX_Jammer_Consolidated_results.xlsx')
    print(consld_fname)
    ...
    if os.path.exists(consld_fname):
        ...
        workbook = Workbook()
        worksheet = workbook.active
        worksheet.append(['Standard','Channel','DataRate','AMPLITUDE']+rx_report_hdngs)
...
##            dut.dut_setStaID(int(staID))
        amplitude_loop(dr,channel,workbook,worksheet,standard,cable_loss,DUT_TestConfigParams, 0)
