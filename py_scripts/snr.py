#-------------------------------------------------------------------------------
# Name:        snr
# Purpose:     This script will calculate the snr value for the specified datarate.
...
def get_10prcnt_per(ch,dr,start_amp,end_amp,step_size,min_per_val,max_per_val,a):
    ...
    for snr in dig_amplitude_list[1:]:
        sigPow = NoiseFloor + snr
        equipment.set_amplitude(streams,str(sigPow))
        if(flag==1):
            raise Exception('RAISED')
        data=[]
        ...
        try:
            ...
            data.append(dr)
        data.append(float(snr))
        if(dig_amplitude_list.index(snr)==1):
            equipment.generate_degrade_waveform(awgn_snr=str(snr),bw=bw)
            DebugPrint("\nDebug1")
            data=data+start_per(modulation,dr,ch,streams,standard,tester,dut,equipment,'awgn_noise')
        else:
            equipment.generate_degrade_waveform(awgn_snr=str(snr),bw=bw)
            DebugPrint("\nDebug2")
            data=data+start_per(modulation,dr,ch,streams,standard,tester,dut,equipment,'awgn_noise')
        ...
        if(float(curr_per)>HIGH_PER_BOUNDARY):
            ...
            snr_amp_dict[ch][dr]=float(curr_amp)
...
def amplitude_loop(dr,ch):
    ...
    for snr in ampl_dict[dr]:
        ...
        data.append(snr)
        if(flg==0):
            if(ampl_dict[dr].index(snr)==0):
                equipment.generate_degrade_waveform(awgn_snr=str(snr),bw=bw)
                start_per(modulation,dr,ch,streams,standard,tester,dut,equipment,'awgn_noise')
            equipment.generate_degrade_waveform(awgn_snr=str(snr),bw=bw)
            data=data+start_per(modulation,dr,ch,streams,standard,tester,dut,equipment,'awgn_noise')
            if(data[4] < 800 ):
                time.sleep(1)
            ...
            if(LOW_PER_BOUNDARY<=float(curr_per)<=HIGH_PER_BOUNDARY):
                ...
                try:
                    ...
                    print e.args
            #elif(int(curr_per)>30):
            elif((int(curr_per)>HIGH_PER_BOUNDARY) or ((int(curr_per)<LOW_PER_BOUNDARY) and ((ampl_dict[dr].index(snr)==(len(ampl_dict[dr])-1))))):
                #for i in range(13):
                #        data.pop()
                #    data=data+start_per(modulation,dr,ch,streams,standard,tester)
                for snr in actual_ampl_dict[dr]:
                    data=[]
                    data.append(standard)
                    data.append(int(ch))
                    try:
                    ...
                    data.append(snr)
                    if(flg==0):
                        # if(actual_ampl_dict[dr].index(snr)==0):
                            # set_amplitude(str(int(snr)+1))
                            # time.sleep(5)
                        equipment.generate_degrade_waveform(awgn_snr=str(snr),bw=bw)
                        data=data+start_per(modulation,dr,ch,streams,standard,tester,dut,equipment,'awgn_noise')
                        print data
                        #if((data[4] < 1200 ) and ((actual_ampl_dict[dr].index(snr)==0) or (actual_ampl_dict[dr].index(snr)==1))):
                        if(data[4] < 800 ):# and ((actual_ampl_dict[dr].index(snr)==0) or (actual_ampl_dict[dr].index(snr)==1))):
                            time.sleep(8)
                            for i in range(num_params_to_pop):
                                data.pop()
                            data=data+start_per(modulation,dr,ch,streams,standard,tester,dut,equipment,'awgn_noise')
                        ...
                        if(LOW_PER_BOUNDARY<=float(curr_per)<=HIGH_PER_BOUNDARY):
                            ...
                            try:
                                ...
                                print e.args
                        elif((actual_ampl_dict[dr].index(snr)==0) and (int(curr_per)>HIGH_PER_BOUNDARY)):
                            return
...
def readLookUpTable(sheet):
    ...
    if (sheet in sheets):
        ...
        for i in range(len(colB)-1):
            snrDict.update({str(colB[i+1].value):colC[i+1].value})

        return snrDict
    else:
        return 'NA'

def writeLookUpTable(snrDict, sheet):
    """ writing Measured SNR and expected sensitivity results to rxPerformanceReport.xlsx """
    reportXlsx = os.path.join(os.getcwd(),'../reports/530_77/rf/rxPerformanceReport.xlsx')
    workbook= load_workbook(reportXlsx)
    sheets = workbook.sheetnames
    ws = workbook['Rev']
    noiseFloor = ws['B3'].value
    ...
    if (sheet in sheets):
        ws = workbook[sheet]
        for datarate in snrDict.keys():
            if ('TBT' != snrDict[datarate]):
                ws['E'+rateDict[datarate]] = round(float(snrDict[datarate]),1)
                ws['F'+rateDict[datarate]] = noiseFloor + round(float(snrDict[datarate]),1)
    workbook.save(reportXlsx)

def save_to_doc(hdng,freq,chn,bw,pktFrameFormat):
    global document_path
    document_path=os.path.join(op_file_path.split(standard)[0],'RX_Characterization_SNR_'+release+'_'+board_num+'_'+rf_num+'.docx')
    try:
    ...
    if('SNR of All DataRates' not in paragraphs):
        document.add_heading('SNR of All DataRates', level=2)
    if(standard=='11ac'):
    ...
    hdr_cells[1].text = 'Measured SNR'
    hdr_cells[2].text = 'Matlab Result'
    if (standard=='11ax'):
        snrDict = readLookUpTable(pktFrameFormat + '_' + bw )
        writeLookUpTable(snr_amp_dict[chn], pktFrameFormat + '_' + bw)
    else:
        snrDict = readLookUpTable(standard + '_' + bw )
        writeLookUpTable(snr_amp_dict[chn], standard + '_' + bw)
    for datarate in data_rate.split(','):
        if(datarate in snr_amp_dict[chn].keys()):
            row_cells = table.add_row().cells
            row_cells[0].text = str(datarate)
            if('TBT' in str(snr_amp_dict[chn][datarate])):
                row_cells[1].text = str(snr_amp_dict[chn][datarate])
            else:
                row_cells[1].text = str(round(float(snr_amp_dict[chn][datarate]),1))
            if (snrDict == 'NA'):
                row_cells[2].text = 'NA'
            else:
                row_cells[2].text = str(snrDict[datarate])
    try:
...
def snr_analysis(DUT_TestConfigParams,dut1):
    #controlPowerSwitch(on_ports_list='8',off_ports_list='8')
    global ftime
    global equipment
    global dut
    ...
    global preamble
    global ampl_dict
    global actual_start_amplt
    global actual_end_amplt
    global step_size
    global worksheet
    global worksheet_dr
    global bold
    ...
    actual_start_amplt=DUT_TestConfigParams.start_amplt       #start_amplt=int(sys.argv[6])
    actual_end_amplt=DUT_TestConfigParams.end_amplt           #end_amplt=int(sys.argv[7])
    step_size=DUT_TestConfigParams.step_size           #step_size=float(sys.argv[8])
    channel=DUT_TestConfigParams.channel               #channel=sys.argv[9]
...
#    dut=eval(dutModel)(com_port)
    ...
    for dr in data_rate.split(','):
        amplitude_list=[]
        start_amplt=snr_dict_range[standard][streams][bw.split('in')[0]][dr][0]
        end_amplt=snr_dict_range[standard][streams][bw.split('in')[0]][dr][1]
        steps_list=list(numpy.arange(start_amplt,end_amplt+1,step_size))
        steps_list.reverse()
        for j in steps_list:
    ...
    for ch in channel.split(','):
        ...
        if(standard=='11ac'):
            ...
            #cnsl_fname=''
        consld_fname=os.path.join(op_file_path,'SNR_Consolidated_'+cnsl_fname+'.xlsx')
        workbook = xlsxwriter.Workbook(consld_fname)
        worksheet = workbook.add_worksheet()
        bold1 = workbook.add_format({'bold': 1})
        if(ch not in snr_amp_dict.keys()):
            snr_amp_dict[ch]={}
        print data_rate
        for dr in data_rate.split(','):
            print dr
            snr_amp_dict[ch][dr]='TBT'
            ...
            dr_file_name=os.path.join(op_file_path,'SNR_'+fname+'.xlsx')
        ...
        try:
            save_to_doc(cnsl_fname,freq,ch,bw,pktFrameFormat)
        except Exception,e:
            print e.args
            pass
    try:
